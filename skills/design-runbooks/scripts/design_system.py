...
# colors.csv encodes surfaces as slot markers, not final values. Every hex neutral in the
# corpus is zero-chroma, which trips gate 22 (minimum 0.005 chroma -- pure greys read as
# flat), and a handful of rows carry pure #FFFFFF / #000000 surfaces, which trip gate 7.
# Gates 7 and 22 stay global and unscoped; the palette is transformed at token-emission
# time instead. colors.csv itself is never modified.
#
# See reference/contract.md section "Neutral tinting" and reference/merged-rules.md C3.

_NEUTRAL_ROLES = (
    "Background", "Foreground", "Card", "Card Foreground",
    "Muted", "Muted Foreground", "Border",
)
_GATE22_CHROMA_FLOOR = 0.005
_TINTED_CHROMA = 0.006          # just above the floor -- a tint, not a cast
_OLED_LIFT_LIGHTNESS = 0.13     # inside the 0.12-0.15 band for OLED halation
_AA_BODY_CONTRAST = 4.5

# Gate 7 exempts the modern-minimal school (Stripe / ElevenLabs) for pure #fff paper.
_MODERN_MINIMAL_MARKERS = ("minimal", "swiss")
...
def _hex_to_linear_rgb(hex_color: str):
    """#RRGGBB -> linear-light sRGB triple, or None if unparseable."""
    if not hex_color:
        return None
    value = hex_color.strip().lstrip("#")
    if len(value) == 3:
...
def _linear_rgb_to_hex(rgb) -> str:
    ...
    for channel in rgb:
        ...
        out.append(max(0, min(255, int(round(encoded * 255)))))
    return "#{0:02X}{1:02X}{2:02X}".format(*out)


def _hex_to_oklch(hex_color: str):
    """#RRGGBB -> (L, C, H_degrees), or None if unparseable."""
    linear = _hex_to_linear_rgb(hex_color)
    if linear is None:
        return None
    lms = [sum(row[i] * linear[i] for i in range(3)) for row in _OKLAB_M1]
    lms = [(v ** (1 / 3)) if v >= 0 else -((-v) ** (1 / 3)) for v in lms]
    lightness, green_red, blue_yellow = [
        sum(row[i] * lms[i] for i in range(3)) for row in _OKLAB_M2
    ]
...
def _oklch_to_hex(lightness: float, chroma: float, hue: float) -> str:
    ...
    lab = (lightness, green_red, blue_yellow)
    lms = [sum(row[i] * lab[i] for i in range(3)) ** 3
           if sum(row[i] * lab[i] for i in range(3)) >= 0
           else -((-sum(row[i] * lab[i] for i in range(3))) ** 3)
           for row in _OKLAB_M2_INV]
    linear = [sum(row[i] * lms[i] for i in range(3)) for row in _LMS_TO_LINEAR]
    return _linear_rgb_to_hex(linear)


def _format_oklch(lightness: float, chroma: float, hue: float) -> str:
    return "oklch({0:.1f}% {1:.4f} {2:.1f})".format(
        lightness * 100, chroma, hue)


def _style_is_modern_minimal(style: dict) -> bool:
    """Gate 7's pure-#fff-paper exception applies to the modern-minimal school only."""
    if not style:
        return False
    blob = " ".join(str(style.get(field, "")) for field in
                    ("Style ID", "Style Category", "Keywords", "Aliases")).lower()
    return any(marker in blob for marker in _MODERN_MINIMAL_MARKERS)


def _tint_neutral_palette(palette: dict, style: dict = None) -> dict:
    """Emit gate-compliant OKLCH surfaces from a colors.csv slot-marker row.

    Tints every zero-chroma neutral toward the palette's primary hue (gate 22), lifts a
    pure-black background off #000 for OLED halation, honours gate 7's modern-minimal
    exception for pure #fff paper, and re-verifies AA body contrast on the POST-transform
    values -- not on the source hex.
    """
    if not palette:
        return palette

    anchor = _hex_to_oklch(palette.get("Primary") or palette.get("Accent") or "")
    anchor_hue = anchor[2] if anchor else 0.0
    minimal_exempt = _style_is_modern_minimal(style)

    tinted = dict(palette)
    emitted, notes = {}, []

    for role in _NEUTRAL_ROLES:
        source = (palette.get(role) or "").strip()
        parsed = _hex_to_oklch(source)
        if parsed is None:
            continue
        lightness, chroma, hue = parsed

        if source.upper() in {"#FFFFFF", "#FFF"} and minimal_exempt and role in {
                "Background", "Card"}:
            notes.append(
                "{0} kept at pure #FFFFFF -- gate 7 exempts the modern-minimal "
                "school for paper.".format(role))
            emitted[role] = source
            continue

        if lightness <= 0.02 and role in {"Background", "Card"}:
            notes.append(
                "{0} lifted off pure black to L {1:.0%} for OLED halation.".format(
                    role, _OLED_LIFT_LIGHTNESS))
            lightness = _OLED_LIFT_LIGHTNESS

        if chroma < _GATE22_CHROMA_FLOOR:
            chroma, hue = _TINTED_CHROMA, anchor_hue

        tinted[role] = _oklch_to_hex(lightness, chroma, hue)
        emitted[role] = _format_oklch(lightness, chroma, hue)

    # AA re-verification on the transformed values, lightening/darkening the ink only.
    for ink, surface in (("Foreground", "Background"),
                         ("Card Foreground", "Card"),
                         ("Muted Foreground", "Muted")):
        if ink not in emitted or surface not in emitted:
            continue
        parsed = _hex_to_oklch(tinted[ink])
        if parsed is None:
            continue
        lightness, chroma, hue = parsed
        surface_is_dark = (_relative_luminance(tinted[surface]) or 0) < 0.5
        step, guard = (0.02 if surface_is_dark else -0.02), 0
        while ((_contrast_ratio(tinted[ink], tinted[surface]) or 0) < _AA_BODY_CONTRAST
               and 0.0 <= lightness <= 1.0 and guard < 50):
            lightness = min(1.0, max(0.0, lightness + step))
            tinted[ink] = _oklch_to_hex(lightness, chroma, hue)
            guard += 1
        if guard:
            emitted[ink] = _format_oklch(lightness, chroma, hue)
            notes.append(
                "{0} lightness adjusted to {1:.0%} to clear AA {2}:1 on {3} after "
                "tinting.".format(ink, lightness, _AA_BODY_CONTRAST, surface))

    tinted["_oklch"] = emitted
    tinted["_tint_notes"] = notes
    tinted["_tinted"] = True
    return tinted


# colors dict key -> colors.csv role name, for looking up the emitted OKLCH value.
_OKLCH_ROLE_BY_KEY = {
...
def _token_value(colors: dict, key: str, hex_val: str) -> str:
    """Normative token value: the tinted OKLCH where one was emitted, else the hex."""
    role = _OKLCH_ROLE_BY_KEY.get(key)
    if not role:
        return hex_val
    return (colors.get("oklch") or {}).get(role) or hex_val
...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None,
        ...
        # colors.csv surfaces are slot markers, not final values. Tint the neutrals at
        # emission time so gates 7 and 22 hold without editing the corpus (C3).
        best_color = _tint_neutral_palette(best_color, best_style)
        ...
        return {
            ...
            "colors": {
                ...
                # OKLCH values for the tinted neutrals (C3). Keyed by colors.csv role
                # name; the hex above is the sRGB fallback for the same color.
                "oklch": best_color.get("_oklch", {}),
                "tint_notes": best_color.get("_tint_notes", []),
            ...
            "typography": {
                # Gate 1 bans Inter as a display face, including as a fallback (C4).
                "heading": best_typography.get("Heading Font", "Figtree"),
...
def format_ascii_box(design_system: dict) -> str:
    ...
    for label, key, css_var in SEMANTIC_COLOR_ENTRIES:
        ...
        token = _token_value(colors, key, hex_val)
        shown = hex_val if token == hex_val else f"{hex_val} {token}"
        content = f"│     {swatch}{label + ':':14s} {shown:32s} ({css_var})"
    ...
    checklist_items = [
        "[ ] No emojis as icons — icons are drawn SVG",
        "[ ] One icon library throughout: Phosphor (glyph lookup available in this skill), or Lucide/Heroicons if you prefer",
...
def format_markdown(design_system: dict) -> str:
    ...
    for label, key, css_var in SEMANTIC_COLOR_ENTRIES:
        ...
        if hex_val:
            token = _token_value(colors, key, hex_val)
            cell = f"`{token}`" if token == hex_val else f"`{token}` (sRGB `{hex_val}`)"
            lines.append(f"| {label} | {cell} | `{css_var}` |")
    if colors.get("notes"):
    ...
    lines.append("- [ ] No emojis as icons — icons are drawn SVG")
    lines.append("- [ ] One icon library throughout: Phosphor (glyph lookup available in this skill), or Lucide/Heroicons if you prefer")
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append("")
    lines.append("- ❌ **Emojis as icons** — icons are drawn SVG, one library throughout")
