...
CSV_CONFIG = {
    ...
    "typography": {
        ...
        "output_cols": ["Font Pairing Name", "Category", "Heading Font", "Body Font", "Mood/Style Keywords", "Best For", "Google Fonts URL", "CSS Import", "Tailwind Config", "Notes", "gate1_fail", "gate1_platform_exempt"]
    },
    "icons": {
        "file": "icons.csv",
...
# --- Gate 1 (display-font ban) retrieval filter ------------------------------
# reference/merged-rules.md section C4. Gate 1 is global and unscoped: Inter, Roboto,
# Open Sans, Poppins, Lato and system defaults are not display faces. Seven of the 74
# pairings carry a banned Heading Font. They are flagged in typography.csv rather than
# deleted, and filtered here so a banned pairing never reaches selection.
#
# Gate 1 governs DISPLAY only, so the body half of a flagged pairing is untouched. Rather
# than discard the row, substitute the heading face: same pairing cluster where possible,
# never a gate-1 face, never the row's own body face, and preferring a face not named in
# the `overused-font` description carried in reference/impeccable-detector-rules.md.

_GATE1_BANNED_DISPLAY = frozenset(
    ("inter", "roboto", "open sans", "poppins", "lato")
)

# Named in the `overused-font` rule description (reference/impeccable-detector-rules.md).
# That rule is not carried -- gate 1 supersedes it -- but these remain poor destinations
# for an automatic substitution.
_GATE1_TIRED_DISPLAY = frozenset(
    ("inter", "roboto", "fraunces", "geist", "plus jakarta sans", "space grotesk")
)



# Display faces an incumbent DESIGN.md has already committed to. A pairing on the gate 1
# ban list is returned intact when it appears here -- an incumbent system outranks the
# catalog -- and always with its gate-1 note attached. Populate before searching.
_GATE1_ALLOWED_DISPLAY_FONTS = []

def _font_family(value):
    """First family in a CSS font stack."""
    return (value or "").split(",")[0].strip()


def _gate1_is_banned(value):
    return _font_family(value).lower() in _GATE1_BANNED_DISPLAY


def _gate1_query_names_font(query, row):
    """Brief evidence: the user explicitly named this pairing's display FACE.

    Deliberately not matched against `Font Pairing Name`. BM25 surfaces a flagged row
    precisely because the query hit its name, so accepting the pairing name as evidence
    would make this filter a no-op. Naming the pairing is not naming the font.
    """
    candidate = _font_family(row.get("Heading Font"))
    if not candidate:
        return False
    return _contains_phrase(_normalize((query or "").lower()),
                            _normalize(candidate.lower()))


def _gate1_keyword_tokens(row):
    blob = "{0} {1}".format(row.get("Mood/Style Keywords", ""), row.get("Best For", ""))
    return frozenset(re.findall(r"[a-z]+", blob.lower()))


def _gate1_head_class(row):
    """Broad class of the pairing's display slot: 'sans', 'serif', 'display', 'mono'."""
    return re.split(r"\s*[+(]", (row.get("Category") or "").strip())[0].strip().lower()


def _gate1_substitute_heading(rows, flagged):
    """Nearest non-banned display face for a flagged pairing, or None."""
    own_body = _font_family(flagged.get("Body Font")).lower()

    def usable(row):
        head = _font_family(row.get("Heading Font")).lower()
        return head and head not in _GATE1_BANNED_DISPLAY and head != own_body

    pools = (
        [r for r in rows if r.get("Category") == flagged.get("Category")],
        [r for r in rows if _gate1_head_class(r) == _gate1_head_class(flagged)],
        rows,
    )
    candidates = []
    for pool in pools:
        candidates = [r for r in pool if usable(r)]
        if candidates:
            break
    if not candidates:
        return None
    ...
    fresh = [r for r in candidates
             if _font_family(r.get("Heading Font")).lower() not in _GATE1_TIRED_DISPLAY]
    candidates = fresh or candidates

    target = _gate1_keyword_tokens(flagged)

    def relevance(row):
        other = _gate1_keyword_tokens(row)
        union = target | other
        return len(target & other) / len(union) if union else 0.0

    return _font_family(max(candidates, key=relevance).get("Heading Font")) or None


def _apply_gate1_policy(rows, results, query, allow_banned=()):
    """Rewrite gate-1-flagged pairings in place of dropping them.

    A flagged row is returned with its banned display face swapped out and a note
    attached, unless the query names the font (brief evidence) or the caller allows it
    (an incumbent DESIGN.md), in which case it is returned intact -- with the note, never
    silently.
    """
    allowed = {str(name).strip().lower() for name in allow_banned if str(name).strip()}
    out = []
    for result in results:
        if not (result.get("gate1_fail") or "").strip():
            out.append(result)
            continue

        row = result
        original = _font_family(row.get("Heading Font"))
        earned = (_gate1_query_names_font(query, row)
                  or original.lower() in allowed
                  or (row.get("Font Pairing Name") or "").strip().lower() in allowed)

        entry = dict(row)
        if earned:
            entry["_gate1"] = {
                "status": "returned-on-evidence",
                "display_font": original,
                "note": (
                    "{0} is on the gate 1 display-font ban list. Returned because the brief "
                    "or an incumbent DESIGN.md names it -- brief evidence earns a category "
        ...
        substitute = _gate1_substitute_heading(rows, row)
        if substitute is None:
            continue

        exempt = (row.get("gate1_platform_exempt") or "").strip()
        entry["Heading Font"] = substitute
        for artifact in ("Google Fonts URL", "CSS Import", "Tailwind Config"):
            if entry.get(artifact):
                entry[artifact] = "[regenerate for {0}] {1}".format(substitute, entry[artifact])
        entry["_gate1"] = {
            "status": "display-substituted",
            "display_font": original,
            "substitute": substitute,
            "platform_exempt": exempt or None,
            "note": (
                "Gate 1 bans {0} as a display face. Body font {1} is unaffected -- gate 1 "
                "governs display only -- so the pairing is kept with {2} in the display "
                "slot. Suggested substitute, not a decision: confirm it against the brief, "
                "and regenerate the font URL/import/config.".format(
                    original, _font_family(row.get("Body Font")), substitute
                )
            ),
        }
        if exempt:
            entry["_gate1"]["note"] += (
                " Platform note: {0} is platform-correct on {1}, where the ban is a "
                "category error.".format(original, exempt)
            )
        out.append(entry)
    return out
...
def search(query, domain=None, max_results=MAX_RESULTS, diagnostics=False):
    ...
    if search_domain == "typography" and results:
        # Gate 1 is global and unscoped -- filter flagged pairings before selection rather
        # than failing them after the fact. reference/merged-rules.md section C4.
        results = _apply_gate1_policy(
            _load_rows_or_empty(filepath), results, query,
            allow_banned=_GATE1_ALLOWED_DISPLAY_FONTS,
        )
        for entry in results:
            entry.pop("gate1_fail", None)
            entry.pop("gate1_platform_exempt", None)

    if search_domain == "icons" and _contains_phrase(_normalize(query.lower()), "lucide"):
        # Coverage limit, not a verdict on Lucide. The curated catalog is Phosphor-only, so
        # naming a specific Lucide glyph here would produce a wrong import. See
        # reference/merged-rules.md section C2.
        results = []
        diagnostic.update({
            "abstained": True,
            "reason": "phosphor-only-catalog",
            "message": (
                "Icon lookup is Phosphor-only; Lucide is a fine choice, I just can't name "
                "specific glyphs for it. Use Lucide's own search."
            ),
        })
