...
> **Extraction note.** Source: `command.md` from
> [vercel-labs/web-interface-guidelines](https://github.com/vercel-labs/web-interface-guidelines)
> at commit `e3d624baaf29dc1fc645aff3e38f03e564d2d6b1`.
> **MIT, Copyright (c) 2025 Vercel Labs** —
> `licenses/vercel-web-interface-guidelines-MIT.txt`.
>
> Every rule below is **verbatim**, in source order, under its source heading. No rule was
> reworded, reordered, renumbered, added or removed. Two mechanical changes only: headings
> demoted `###` → `##` (upstream's `## Rules` parent is dropped), and upstream's frontmatter,
> `$ARGUMENTS` preamble and `## Output Format` section removed — Runbook 3 owns the finding
> format. See `MANIFEST.md` § 12.
