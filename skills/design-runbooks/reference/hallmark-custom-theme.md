...
> contributors (`licenses/hallmark-MIT.txt`). File: `skills/hallmark/references/custom-theme.md`.
> Everything below the next rule is **verbatim**, in source order — the file is almost entirely
> lists, tables, palettes and stamps, so nothing was paraphrased and nothing was dropped.
> Cross-references to hallmark files not extracted here (`color.md`, `typography.md`,
> `anti-patterns.md`, `microinteractions.md`, `tokens.css`, `SKILL.md`) are left in place as
> provenance; they resolve in the upstream tree, not in this skill.
> The blockquote below is extraction commentary, not source text.

> **The escape-hatch pattern, distilled.** Three properties keep this a rarely-taken branch
> rather than a second default — each one is stated in the source text that follows:
> 1. the fork is surfaced **only** on an enumerated trigger-signal list (§ *When to surface this
>    fork*), and *"If **none** of the signals fires, do not mention the fork at all."*;
> 2. the confirmation question **defaults to the incumbent** — *"**Default to catalog** — silence
>    routes to catalog, not custom."*;
> 3. the floor does not move — *"every slop-test gate fires unchanged at every depth."*
>
> Depth ladder: **catalog** (default, most briefs) → **custom · tuned** (signals 1–4: new palette
> + pairing, catalog structures kept) → **custom · bespoke** (signal 5 only: structure designed
> from first principles). *"Bespoke is rare."*
